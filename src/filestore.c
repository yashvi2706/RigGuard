...
// ROLES definition (declared extern in utils.h)
const char *ROLES[NUM_ROLES] = {
    "Rig_Commander",
    "Fire_Safety_Officer",
    "Drilling_Operator",
...
void print_messages(const char *for_user) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        char *tok = strtok(tmp, ",");
        if (!tok) continue;
        strncpy(ts, tok, MAX_STR);
        tok = strtok(NULL, ","); if (!tok) continue;
        strncpy(sender, tok, MAX_STR);
        tok = strtok(NULL, ","); if (!tok) continue;
        strncpy(receiver, tok, MAX_STR);
        tok = strtok(NULL, "\n"); if (!tok) continue;
        strncpy(msg, tok, MAX_MSG);
